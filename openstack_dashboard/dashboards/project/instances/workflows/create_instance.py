...
class SetInstanceDetailsAction(workflows.Action):
    ...
    def __init__(self, request, context, *args, **kwargs):
        ...
        boot_from_volume_regions = getattr(
            settings, 'BOOT_FROM_VOLUME_REGIONS', ['RegionOne'])
        if request.user.services_region in boot_from_volume_regions:
        ...
        try:
            ...
            if port_profiles_supported:
                ...
                if ports_failing_deletes:
                    ...
                    msg = ('Port cleanup failed for these port-ids (%s).'
                           % ports_str)
                    LOG.exception(msg)
            exceptions.handle(request)
        return False
