...
ZONE_BASED_NETWORK_SCHEDULER = getattr(
    settings, 'ZONE_BASED_NETWORK_SCHEDULER', False)


class SetInstanceDetailsAction(workflows.Action):
    if ZONE_BASED_NETWORK_SCHEDULER:
    ...
    def flavors(self, request):
        try:
            ...
            for flavor in api.nova.flavor_list(request):
                if flavor.ram >= 1024 and flavor.disk > 5:
    ...
    def populate_availability_zone_choices(self, request, context):
        ...
        zone_list.sort()
        try:
            if context.get('project_id') not in \
               settings.DISABLE_ZONES_EXCEPT_TENANT:
                for z in settings.DISABLE_ZONES:
                    if z == '':
                        continue
                    zone_list = [x for x in zone_list if x[0] != z]

            additional_zones = settings.ADDITIONAL_ZONES
            additional_zones.reverse()
            for additional_zone in additional_zones:
                if additional_zone is not '':
                    zone_list.insert(0, (additional_zone, additional_zone))
        except Exception:
            pass
        if not zone_list:
    ...
    def datastores(self, request):
        try:
            datastore_list = api.trove.datastore_list(request)
            datastore_list = \
                [x for x in datastore_list if x.name != 'postgresql']
    ...
    def populate_backup_choices(self, request, context):
        try:
            if context.get('backup'):
                self.fields['initial_state'].initial = 'backup'
            else:
                self.fields['initial_state'].initial = 'None'
